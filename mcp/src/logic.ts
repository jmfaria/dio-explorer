...
export interface PlanoTrilha {
  ...
  numero_de_modulos: number;
  plano_de_estudos: {
    fase_1_fundamentos: Modulo[];
    fase_2_pratica: Modulo[];
    fase_3_avancado: Modulo[];
  };
  badges_disponiveis: string[];
  lives_ao_vivo: { titulo: string; data: string; instrutor: string }[];
  dicas_de_estudo: string[];
}
...
export interface Desafio {
  ...
  objetivo: string[];
  entrada: string;
  saida_esperada: { entrada: string; saida: string }[];
  dicas: string[];
...
const { trilhas } = require(join(__dirname, '../../data/trilhas-dio.json')) as { trilhas: Trilha[] };
...
const TEMPO_POR_NIVEL: Record<string, string> = {
  Básico: '4–6h',
  Intermediário: '6–8h',
  Avançado: '8–12h',
};

function gerarModulos(tecnologia: string, nivel: string, total: number): Modulo[] {
...
function gerarDicas(tecnologia: string): string[] {
  return [
...
function gerarSufixo(n: number): string {
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  return Array.from({ length: n }, () => chars[Math.floor(Math.random() * chars.length)]).join('');
}

function gerarCompetencias(tecnologia: string, nivel: string): string[] {
  const base = [
...
export function buscarTrilha(tecnologia: string): PlanoTrilha | null {
...
export function gerarCertificado(nome: string, trilhaQuery: string): ResultadoCertificado {
  const query = trilhaQuery.toLowerCase();
  const trilha = trilhas.find(
    t => t.nome.toLowerCase().includes(query) || t.tecnologia.toLowerCase().includes(query),
  );
...
export function normalizarNivel(raw: string): string | null {
  return NIVEL_MAP[raw.toLowerCase()] ?? null;
}

const XP_POR_NIVEL: Record<string, [number, number]> = {
  Básico: [200, 700],
  Intermediário: [700, 1400],
  Avançado: [1400, 2000],
};

const TEMPO_DESAFIO: Record<string, string> = {
  Básico: '30–60 minutos',
  Intermediário: '1–3 horas',
  Avançado: '3–6 horas',
};

const BANCOS_DE_DESAFIOS: Record<string, Array<{
  titulo: string;
  descricao: string;
  objetivo: string[];
  entrada: string;
